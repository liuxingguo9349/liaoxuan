...
function evaluateNonDoctor(data) {
  ...
  if (data.undergradTier === "main") {
    if (data.top60 === "yes" && data.party && data.award) {
      return buildResult(
        "province",
        "中高",
        [...reasons, "38所主池这一档的省直更像是：班级/专业成绩前60% + 党员 + 校级荣誉；你已经满足这组核心条件。"],
        risks,
        "因为你本科在38所主池这一档，同时满足班级/专业成绩前60%、党员和校级荣誉，所以最像省直档。学生干部和参军入伍会继续增强稳定性，但省直核心仍更像卡党员、成绩和校奖。",
        "大概率有资格报考第一批"
      );
    }
