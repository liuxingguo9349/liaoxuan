...
function evaluateNonDoctor(data) {
  ...
  let effectiveTier = data.undergradTier;
  ...
  if (data.anyQingbei && data.undergradTier === "newd1") {
    effectiveTier = "main";
    reasons.push("虽然你的本科是新双一流，但因为当前报考路径里还有清北经历，所以这里先按比新双一流上调一档后的“38所主池”口径来判断。");
  } else if (data.anyQingbei && data.undergradTier === "main") {
    effectiveTier = "qingbei";
    reasons.push("虽然你的本科在38所主池这一档，但因为当前报考路径里还有清北经历，所以这里先按清北放宽档来判断。");
  }

  if (effectiveTier === "qingbei") {
    if (data.top60 === "yes" && data.party && hasAnyExtra) {
      return buildResult(
        "province",
        "高",
        [...reasons, "清北这一档可理解为在38所主池基础上再放宽一项；你已经满足党员、班级/专业成绩前60%和至少一个补位项，因此结果稳稳落在省直附近。"],
        risks,
        "因为你当前这条非博士报考路径已经可以按清北放宽档来判断，同时满足班级/专业成绩前60%、党员，再加学生干部、校级荣誉、参军入伍中的至少一项，所以最像省直档。",
        "大概率有资格报考第一批"
      );
    }
    ...
    if (data.top60 === "yes" && data.party) {
      return buildResult(
        "province",
        "高",
        [...reasons, "清北这一档相当于把38所主池整体上抬一档；因此在班级/专业成绩前60%还在、党员也在时，通常就已经够到省直。"],
        risks,
        "因为你当前这条非博士报考路径已经可以按清北放宽档来判断，而且班级/专业成绩前60%和党员都满足，所以结果更像省直。学生干部、校级荣誉、参军入伍会让这个判断更稳，但不是这一档的必需项。",
        "大概率有资格报考第一批"
      );
    }
    ...
    if (data.top60 === "yes" && data.highlightCount >= 1) {
      return buildResult(
        "city",
        "中高",
        [...reasons, "清北这一档相当于把38所主池整体上抬一档；当前虽然缺党员，但只要四项里已有一项，通常也会上探到市直附近。"],
        risks,
        "因为你当前这条非博士报考路径已经可以按清北放宽档来判断，且班级/专业成绩前60%还在；虽然没有党员，但党员、学生干部、校级荣誉、参军入伍四项里至少已经有一项，所以更像沈阳/大连市直。",
        "大概率有资格报考第一批"
      );
    }
  }
  ...
  if (effectiveTier === "main") {
    if (data.top60 === "yes" && data.party && hasAnyExtra) {
      return buildResult(
        ...
        risks,
        "因为你当前这条非博士报考路径按38所主池这一档来判断，同时满足班级/专业成绩前60%、党员，再加学生干部、校级荣誉、参军入伍中的任意一项，所以最像省直档。这三项在这一层按同权补位处理。",
        "大概率有资格报考第一批"
      );
    }
    ...
    if (data.top60 === "yes" && data.party) {
      return buildResult(
        ...
        risks,
        "因为你当前这条非博士报考路径按38所主池这一档来判断，同时班级/专业成绩前60%和党员都满足，所以更像沈阳/大连市直。学生干部、校级荣誉、参军入伍在这一层主要起补强作用。",
        "大概率有资格报考第一批"
      );
    }
    ...
    if (data.top60 === "yes" && data.highlightCount >= 1) {
      return buildResult(
        ...
        risks,
        "因为你当前这条非博士报考路径按38所主池这一档来判断，班级/专业成绩前60%也保住了；虽然还不够稳到市直，但只要党员、学生干部、校级荣誉、参军入伍里有一项，就比较像沈大区直。",
        "大概率有资格报考第一批"
      );
    }
  }

  if (effectiveTier === "newd1") {
    if (data.top60 === "yes" && data.party && hasAnyExtra) {
      return buildResult(
        "city",
        "中",
        ...
        risks,
        "因为你当前这条非博士报考路径按新双一流这一档来判断，同时满足班级/专业成绩前60%、党员，以及学生干部、校级荣誉、参军入伍中的至少一项，所以更像沈阳/大连市直。",
        "大概率有资格报考第一批"
      );
    }
    ...
    if (data.top60 === "yes" && data.highlightCount >= 2) {
      return buildResult(
        ...
        risks,
        "因为你当前这条非博士报考路径按新双一流这一档来判断，班级/专业成绩前60%也还在；虽然还不够稳到市直，但党员、学生干部、校级荣誉、参军入伍里已经满足两项，所以更像沈大区直。",
        "大概率有资格报考第一批"
      );
    }
  }
